...
(function () {
	...
	var bitcoinjs = window.bitcoinjs = function () { };

	/* public vars */
	bitcoinjs.pub = 0x00;
	bitcoinjs.priv = 0x80;
	bitcoinjs.multisig = 0x05;
	...
	bitcoinjs.hostname	= ((document.location.hostname.split(".")[(document.location.hostname.split(".")).length-1]) == 'onion') ? 'coinbin3ravkwb24f7rmxx6w3snkjw45jhs5lxbh3yfeg3vpt6janwqd.onion' : 'coinb.in';
	bitcoinjs.host = ('https:'==document.location.protocol?'https://':'http://')+bitcoinjs.hostname+'/api/';
	bitcoinjs.uid = '1';
	bitcoinjs.key = '12345678901234567890123456789012';

	/* start of address functions */

	/* generate a private and public keypair, with address and WIF address */
	bitcoinjs.newKeys = function(input){
	...
	bitcoinjs.newPubkey = function(hash){
		var privateKeyBigInt = BigInteger.fromByteArrayUnsigned(Crypto.util.hexToBytes(hash));
		var curve = BitEllipticCurve.getSECCurveByName("secp256k1");

		var curvePt = curve.getG().multiply(privateKeyBigInt);
		var x = curvePt.getX().toBigInteger();
		var y = curvePt.getY().toBigInteger();

		var publicKeyBytes = BitEllipticCurve.integerToBytes(x, 32);
		publicKeyBytes = publicKeyBytes.concat(BitEllipticCurve.integerToBytes(y,32));
		publicKeyBytes.unshift(0x04);

		if(bitcoinjs.compressed==true){
			var publicKeyBytesCompressed = BitEllipticCurve.integerToBytes(x,32)
			if (y.isEven()){
	...
	bitcoinjs.pubkey2address = function(h, byte){
		var r = ripemd160(Crypto.SHA256(Crypto.util.hexToBytes(h), {asBytes: true}));
		r.unshift(byte || bitcoinjs.pub);
		var hash = Crypto.SHA256(Crypto.SHA256(r, {asBytes: true}), {asBytes: true});
		var checksum = hash.slice(0, 4);
		return bitcoinjs.base58encode(r.concat(checksum));
	}

	/* provide a scripthash and return address */
	bitcoinjs.scripthash2address = function(h){
		var x = Crypto.util.hexToBytes(h);
		x.unshift(bitcoinjs.pub);
		var r = x;
		r = Crypto.SHA256(Crypto.SHA256(r,{asBytes: true}),{asBytes: true});
		var checksum = r.slice(0,4);
		return bitcoinjs.base58encode(x.concat(checksum));
	}

	/* new multisig address, provide the pubkeys AND required signatures to release the funds */
	bitcoinjs.pubkeys2MultisigAddress = function(pubkeys, required) {
		var s = bitcoinjs.script();
		...
		x.unshift(bitcoinjs.multisig);
		var r = x;
		r = Crypto.SHA256(Crypto.SHA256(r, {asBytes: true}), {asBytes: true});
		var checksum = r.slice(0,4);
		var redeemScript = Crypto.util.bytesToHex(s.buffer);
		var address = bitcoinjs.base58encode(x.concat(checksum));
	...
	*/
	bitcoinjs.simpleHodlAddress = function(pubkey, checklocktimeverify) {
		...
		if (checklocktimeverify <= 16 && checklocktimeverify >= 1) {
			...
			s.writeBytes(bitcoinjs.numToScriptNumBytes(checklocktimeverify));
		}
	...
	bitcoinjs.segwitAddress = function(pubkey){
	...
	bitcoinjs.bech32Address = function(pubkey){
		var program = ripemd160(Crypto.SHA256(Crypto.util.hexToBytes(pubkey), {asBytes: true}), {asBytes: true});
		var address = bitcoinjs.bech32_encode(bitcoinjs.bech32.hrp, [bitcoinjs.bech32.version].concat(bitcoinjs.bech32_convert(program, 8, 5, true)));
	...
	bitcoinjs.bech32redeemscript = function(address){
		var r = false;
		var decode = bitcoinjs.bech32_decode(address);
		if(decode){
			decode.data.shift();
			return Crypto.util.bytesToHex(bitcoinjs.bech32_convert(decode.data, 5, 8, false));
		}
		return r;
	}

	/* provide a privkey and return an WIF  */
	bitcoinjs.privkey2wif = function(h){
	...
	bitcoinjs.wif2privkey = function(wif){
		var compressed = false;
		var decode = bitcoinjs.base58decode(wif);
	...
	bitcoinjs.wif2pubkey = function(wif){
		var compressed = bitcoinjs.compressed;
		var r = bitcoinjs.wif2privkey(wif);
		bitcoinjs.compressed = r['compressed'];
		var pubkey = bitcoinjs.newPubkey(r['privkey']);
		bitcoinjs.compressed = compressed;
		return {'pubkey':pubkey,'compressed':r['compressed']};
	}

	/* convert a wif to a address */
	bitcoinjs.wif2address = function(wif){
		var r = bitcoinjs.wif2pubkey(wif);
		return {'address':bitcoinjs.pubkey2address(r['pubkey']), 'compressed':r['compressed']};
	}

	/* decode or validate an address and return the hash */
	bitcoinjs.addressDecode = function(addr){
		try {
			var bytes = bitcoinjs.base58decode(addr);
			...
			if (checksum+"" == back+"") {
				...
				if(o.version==bitcoinjs.pub){ // standard address
					o.type = 'standard';

				} else if (o.version==bitcoinjs.multisig) { // multisig address
					o.type = 'multisig';

				} else if (o.version==bitcoinjs.priv){ // wifkey
					o.type = 'wifkey';
			...
			bech32rs = bitcoinjs.bech32redeemscript(addr);
			if(bech32rs){
	...
	bitcoinjs.addressBalance = function(address, callback){
		bitcoinjs.ajax(bitcoinjs.host+'?uid='+bitcoinjs.uid+'&key='+bitcoinjs.key+'&setmodule=addresses&request=bal&address='+address+'&r='+Math.random(), callback, "GET");
	}

	/* decompress an compressed public key */
	bitcoinjs.pubkeydecompress = function(pubkey) {
	...
	bitcoinjs.bech32_polymod = function(values) {
		var chk = 1;
	...
	bitcoinjs.bech32_hrpExpand = function(hrp) {
		var ret = [];
		var p;
	...
	bitcoinjs.	bech32_verifyChecksum = function(hrp, data) {
		return bitcoinjs.bech32_polymod(bitcoinjs.bech32_hrpExpand(hrp).concat(data)) === 1;
	}

	bitcoinjs.bech32_createChecksum = function(hrp, data) {
		var values = bitcoinjs.bech32_hrpExpand(hrp).concat(data).concat([0, 0, 0, 0, 0, 0]);
		var mod = bitcoinjs.bech32_polymod(values) ^ 1;
		var ret = [];
		for (var p = 0; p < 6; ++p) {
	...
	bitcoinjs.bech32_encode = function(hrp, data) {
		var combined = data.concat(bitcoinjs.bech32_createChecksum(hrp, data));
		var ret = hrp + '1';
		for (var p = 0; p < combined.length; ++p) {
			ret += bitcoinjs.bech32.charset.charAt(combined[p]);
		}
		return ret;
	}

	bitcoinjs.bech32_decode = function(bechString) {
		var p;
		var has_lower = false;
		var has_upper = false;
		...
		for (p = pos + 1; p < bechString.length; ++p) {
			var d = bitcoinjs.bech32.charset.indexOf(bechString.charAt(p));
			if (d === -1) {
				return null;
			}
			data.push(d);
		}
		if (!bitcoinjs.bech32_verifyChecksum(hrp, data)) {
			return null;
		}
		return {
			hrp: hrp,
	...
	bitcoinjs.bech32_convert = function(data, inBits, outBits, pad) {
		var value = 0;
		var bits = 0;
	...
	bitcoinjs.testdeterministicK = function() {
		...
		for (i = 0; i < test_vectors.length; i++) {
			...
			var wif = bitcoinjs.privkey2wif(test_vectors[i]['privkey']);
	...
	bitcoinjs.hd = function(data){
		...
		r.parse = function() {
			...
			if(typeof(data) == 'string'){
				var decoded = bitcoinjs.base58decode(data);
			...
			if(bytes && bytes.length>0) {
 				r.version = bitcoinjs.uint(bytes.slice(0, 4) , 4);
 				r.depth = bitcoinjs.uint(bytes.slice(4, 5) ,1);
				r.parent_fingerprint = bytes.slice(5, 9);
				r.child_index = bitcoinjs.uint(bytes.slice(9, 13), 4);
 				r.chain_code = bytes.slice(13, 45);
				r.key_bytes = bytes.slice(45, 78);

				var c = bitcoinjs.compressed; // get current default
				...
				if(r.key_bytes[0] == 0x00) {
					...
					var pubkey = bitcoinjs.newPubkey(privkeyHex);

					r.keys = {'privkey':privkeyHex,
						'pubkey':pubkey,
						'address':bitcoinjs.pubkey2address(pubkey),
						'wif':bitcoinjs.privkey2wif(privkeyHex)};
					...
					r.keys = {'pubkey': pubkeyHex,
						'address':bitcoinjs.pubkey2address(pubkeyHex)};
				} else {
					r.type = 'invalid';
				}
				...
				bitcoinjs.compressed = c; // reset to default
			}
		...
		r.derive_path = function(path) {
			...
			var p = path.split('/');
			var hdp = bitcoinjs.clone(this);  // clone hd path
		...
		r.derive = function(i){
			...
			i = (i)?i:0;
			var blob = (Crypto.util.hexToBytes(this.keys.pubkey)).concat(bitcoinjs.numToBytes(i,4).reverse());
			...
			var ecparams = BitEllipticCurve.getSECCurveByName("secp256k1");
			var curve = ecparams.getCurve();

			var k, key, pubkey, o;

			o = bitcoinjs.clone(this);
			o.chain_code = ir;
			o.child_index = i;
			...
			if(this.type=='private'){
				...
				pubkey = bitcoinjs.newPubkey(key);

				o.keys = {'privkey':key,
					'pubkey':pubkey,
					'wif':bitcoinjs.privkey2wif(key),
					'address':bitcoinjs.pubkey2address(pubkey)};
		...
		r.master = function(pass) {
			var seed = (pass) ? Crypto.SHA256(pass) : bitcoinjs.newPrivkey();
			...
			return hd.make({'depth':0,
				...
				'pubkey':bitcoinjs.newPubkey(I.slice(0, 64))});
		}
		...
		r.make = function(data){ // { (int) depth, (array) parent_fingerprint, (int) child_index, (byte array) chain_code, (hex str) privkey, (hex str) pubkey}
			...
			//child index
			k = k.concat((bitcoinjs.numToBytes(data.child_index, 4)).reverse());
			...
			//encode xprv key
			if(data.privkey){
				var prv = (bitcoinjs.numToBytes(bitcoinjs.hdkey.prv, 4)).reverse();
				prv = prv.concat(k);
				prv.push(0x00);
				...
				o.privkey = bitcoinjs.base58encode(ret);
			}

			//encode xpub key
			if(data.pubkey){
				var pub = (bitcoinjs.numToBytes(bitcoinjs.hdkey.pub, 4)).reverse();
				pub = pub.concat(k);
				...
				o.pubkey = bitcoinjs.base58encode(ret);
			}
			return o;
		}
	...
	bitcoinjs.script = function(data) {
		var r = {};
		...
		} else if (bitcoinjs.isArray(data)) {
			r.buffer = data;
		} else if (data instanceof bitcoinjs.script) {
		...
		r.decodeRedeemScript = function(script){
			var r = false;
			try {
				var s = bitcoinjs.script(Crypto.util.hexToBytes(script));
				if((s.chunks.length>=3) && s.chunks[s.chunks.length-1] == 174){//OP_CHECKMULTISIG
					...
					r.pubkeys = pubkeys;
					var multi = bitcoinjs.pubkeys2MultisigAddress(pubkeys, r.signaturesRequired);
					...
					r.type = "segwit__";
					var rs = Crypto.util.bytesToHex(s.buffer);
					r.address = bitcoinjs.pubkey2address(rs, bitcoinjs.multisig);
					r.redeemscript = rs;
					...
					r.checklocktimeverify = bitcoinjs.bytesToNum(s.chunks[0].slice());
					r.address = bitcoinjs.simpleHodlAddress(r.pubkey, r.checklocktimeverify).address;
					var rs = Crypto.util.bytesToHex(s.buffer);
					r.redeemscript = rs;
					r.type = "hodl__";
				}
		...
		r.spendToScript = function(address){
			var addr = bitcoinjs.addressDecode(address);
			var s = bitcoinjs.script();
			if(addr.type == "bech32"){
				s.writeOp(0);
				s.writeBytes(Crypto.util.hexToBytes(addr.redeemscript));
			} else if(addr.version==bitcoinjs.multisig){ // multisig address
	...
	bitcoinjs.transaction = function() {
		...
		r.addinput = function(txid, index, script, sequence){
			...
			o.script = bitcoinjs.script(script||[]);
		...
		r.addstealth = function(stealth, value){
			var ephemeralKeyBigInt = BigInteger.fromByteArrayUnsigned(Crypto.util.hexToBytes(bitcoinjs.newPrivkey()));
			var curve = BitEllipticCurve.getSECCurveByName("secp256k1");

			var p = BitEllipticCurve.fromHex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F");
			var a = BigInteger.ZERO;
			var b = BitEllipticCurve.fromHex("7");
			var calccurve = new BitEllipticCurve.CurveFp(p, a, b);
			...
			var sendaddress = bitcoinjs.pubkey2address(Crypto.util.bytesToHex(addressPt.getEncoded(true)));


			var OPRETBytes = [6].concat(Crypto.util.randomBytes(4)).concat(ephemeralPt.getEncoded(true)); // ephemkey data
			var q = bitcoinjs.script();
		...
		r.listUnspent = function(address, callback) {
			bitcoinjs.ajax(bitcoinjs.host+'?uid='+bitcoinjs.uid+'&key='+bitcoinjs.key+'&setmodule=addresses&request=unspent&address='+address+'&r='+Math.random(), callback, "GET");
		}

		/* list transaction data */
		r.getTransaction = function(txid, callback) {
			bitcoinjs.ajax(bitcoinjs.host+'?uid='+bitcoinjs.uid+'&key='+bitcoinjs.key+'&setmodule=bitcoin&request=gettransaction&txid='+txid+'&r='+Math.random(), callback, "GET");
		}
				...
				if(unspent){
					for(i=1;i<=unspent.childElementCount;i++){
						...
						if(segwit){
							...
							s.writeOp(0);
							s.writeBytes(bitcoinjs.numToBytes(u.getElementsByTagName("value")[0].childNodes[0].nodeValue*1, 8));
		...
		r.addUnspentAndSign = function(wif, callback){
			var self = this;
			var address = bitcoinjs.wif2address(wif);
		...
		r.broadcast = function(callback, txhex){
			...
			bitcoinjs.ajax(bitcoinjs.host+'?uid='+bitcoinjs.uid+'&key='+bitcoinjs.key+'&setmodule=bitcoin&request=sendrawtransaction', callback, "POST", ["rawtx="+tx]);
		}

		/* generate the transaction hash to sign from a transaction input */
		r.transactionHash = function(index, sigHashType) {

			var clone = bitcoinjs.clone(this);
			var shType = sigHashType || 1;

			/* black out all other ins, except this one */
			for (var i = 0; i < clone.ins.length; i++) {
				if(index!=i){
					clone.ins[i].script = bitcoinjs.script();
				}
			}

			var extract = this.extractScriptKey(index);
			clone.ins[index].script = bitcoinjs.script(extract['script']);
			...
			if((clone.ins) && clone.ins[index]){
				...
				buffer = buffer.concat(bitcoinjs.numToBytes(parseInt(shType), 4));
		...
		r.transactionHashSegWitV0 = function(index, sigHashType){
			...
			*/
			...
			var value = bitcoinjs.numToBytes(extract['value'], 8);

			// start

			var zero = bitcoinjs.numToBytes(0, 32);
			var version = bitcoinjs.numToBytes(parseInt(this.version), 4);
			...
			if(!(sigHashType >= 80)){	// not sighash anyonecanpay
				for(var i = 0; i < this.ins.length; i++){
					...
					bufferTmp = bufferTmp.concat(bitcoinjs.numToBytes(this.ins[i].outpoint.index, 4));
				}
			}
			...
			if(!(sigHashType >= 80) && sigHashType != 2 && sigHashType != 3){ // not sighash anyonecanpay & single & none
				for(var i = 0; i < this.ins.length; i++){
					bufferTmp = bufferTmp.concat(bitcoinjs.numToBytes(this.ins[i].sequence, 4));
				}
			}
			...
			outpoint = outpoint.concat(bitcoinjs.numToBytes(this.ins[index].outpoint.index, 4));

			var nsequence = bitcoinjs.numToBytes(this.ins[index].sequence, 4);
			var hashOutputs = zero;
			var bufferTmp = [];
			if(sigHashType != 2 && sigHashType != 3){		// not sighash single & none
				for(var i = 0; i < this.outs.length; i++ ){
					bufferTmp = bufferTmp.concat(bitcoinjs.numToBytes(this.outs[i].value, 8));
					bufferTmp = bufferTmp.concat(bitcoinjs.numToVarInt(this.outs[i].script.buffer.length));
				...
				bufferTmp = bufferTmp.concat(bitcoinjs.numToBytes(this.outs[index].value, 8));
				bufferTmp = bufferTmp.concat(bitcoinjs.numToVarInt(this.outs[i].script.buffer.length));
				bufferTmp = bufferTmp.concat(this.outs[index].script.buffer);
				hashOutputs = Crypto.SHA256(Crypto.SHA256(bufferTmp, {asBytes: true}), {asBytes: true});
			}

			var locktime = bitcoinjs.numToBytes(this.lock_time, 4);
			var sighash = bitcoinjs.numToBytes(sigHashType, 4);
		...
		r.extractScriptKey = function(index) {
			if(this.ins[index]){
				if((this.ins[index].script.chunks.length==5) && this.ins[index].script.chunks[4]==172 && bitcoinjs.isArray(this.ins[index].script.chunks[2])){ //OP_CHECKSIG
					...
					if((this.ins[index].script.chunks[2]) && this.ins[index].script.chunks[2].length==8){
						value = bitcoinjs.bytesToNum(this.ins[index].script.chunks[2]);  // value found encoded in transaction (THIS IS NON STANDARD)
					}
		...
		r.transactionSig = function(index, wif, sigHashType, txhash){
			...
			if(hash){
				var curve = BitEllipticCurve.getSECCurveByName("secp256k1");
				var key = bitcoinjs.wif2privkey(wif);
		...
		r.signinput = function(index, wif, sigHashType){
			var key = bitcoinjs.wif2pubkey(wif);
			var shType = sigHashType || 1;
		...
		r.signmultisig = function(index, wif, sigHashType){
			...
			function scriptListPubkey(redeemScript){
				...
				for(var i=1;i<redeemScript.chunks.length-2;i++){
					r[i] = Crypto.util.hexToBytes(bitcoinjs.pubkeydecompress(Crypto.util.bytesToHex(redeemScript.chunks[i])));
				}
				return r;
			}
			...
			var pubkeyList = scriptListPubkey(bitcoinjs.script(redeemScript));
			...
			sigsList[bitcoinjs.countObject(sigsList)+1] = signature;
			...
			for(x in pubkeyList){
				for(y in sigsList){
					...
					if(bitcoinjs.verifySignature(sighash, sigsList[y], pubkeyList[x])){
		...
		r.signsegwit = function(index, wif, sigHashType){
			...
			var wif2 = bitcoinjs.wif2pubkey(wif);
			var segwit = bitcoinjs.segwitAddress(wif2['pubkey']);
			var bech32 = bitcoinjs.bech32Address(wif2['pubkey']);
			...
			if((segwit['redeemscript'] == Crypto.util.bytesToHex(this.ins[index].script.chunks[0])) || (bech32['redeemscript'] == Crypto.util.bytesToHex(this.ins[index].script.chunks[0]))){
				...
				if(txhash.result == 1){
					...
					if(!bitcoinjs.isArray(this.witness)){
						this.witness = [];
					}
					...
					for(var i = 0; i < this.ins.length; i++){
						for(var y = 0; y < this.witness.length; y++){
							if(!witness_used.includes(y)){
								var sw = bitcoinjs.segwitAddress(this.witness[y][1]);
								var b32 = bitcoinjs.bech32Address(this.witness[y][1]);
								var rs = '';
								...
								if((sw['redeemscript'] == rs) || (b32['redeemscript'] == rs)){
									...
									if(b32['redeemscript'] == rs){
										this.ins[index].script = bitcoinjs.script();
									}
									break;
								}
							}
		...
		r.sign = function(wif, sigHashType){
			...
			for (var i = 0; i < this.ins.length; i++) {
				...
				var w2a = bitcoinjs.wif2address(wif);
				var script = bitcoinjs.script();
		...
		r.serialize = function(){
			var buffer = [];
			buffer = buffer.concat(bitcoinjs.numToBytes(parseInt(this.version),4));

			if(bitcoinjs.isArray(this.witness)){
				buffer = buffer.concat([0x00, 0x01]);
			}

			buffer = buffer.concat(bitcoinjs.numToVarInt(this.ins.length));
			for (var i = 0; i < this.ins.length; i++) {
				var txin = this.ins[i];
				buffer = buffer.concat(Crypto.util.hexToBytes(txin.outpoint.hash).reverse());
				buffer = buffer.concat(bitcoinjs.numToBytes(parseInt(txin.outpoint.index),4));
				var scriptBytes = txin.script.buffer;
				buffer = buffer.concat(bitcoinjs.numToVarInt(scriptBytes.length));
				buffer = buffer.concat(scriptBytes);
				buffer = buffer.concat(bitcoinjs.numToBytes(parseInt(txin.sequence),4));
			}
			buffer = buffer.concat(bitcoinjs.numToVarInt(this.outs.length));

			for (var i = 0; i < this.outs.length; i++) {
				var txout = this.outs[i];
 				buffer = buffer.concat(bitcoinjs.numToBytes(txout.value,8));
				var scriptBytes = txout.script.buffer;
				buffer = buffer.concat(bitcoinjs.numToVarInt(scriptBytes.length));
				buffer = buffer.concat(scriptBytes);
			}

			if((bitcoinjs.isArray(this.witness)) && this.witness.length>=1){
				for(var i = 0; i < this.witness.length; i++){
	 				buffer = buffer.concat(bitcoinjs.numToVarInt(this.witness[i].length));
					for(var x = 0; x < this.witness[i].length; x++){
		 				buffer = buffer.concat(bitcoinjs.numToVarInt(Crypto.util.hexToBytes(this.witness[i][x]).length));
						buffer = buffer.concat(Crypto.util.hexToBytes(this.witness[i][x]));
					}
				}
			}

			buffer = buffer.concat(bitcoinjs.numToBytes(parseInt(this.lock_time),4));
		...
		r.deserialize = function(buffer){
			...
			var obj = new bitcoinjs.transaction();
			...
			for (var i = 0; i < ins; i++) {
				obj.ins.push({
					...
					script: bitcoinjs.script(readVarString()),
			...
			for (var i = 0; i < outs; i++) {
				obj.outs.push({
					value: bitcoinjs.bytesToNum(readBytes(8)),
					script: bitcoinjs.script(readVarString())
				});
			}
			...
			if(witness == true){
				for (i = 0; i < ins; ++i) {
					...
					for(var y = 0; y < count; y++){
						...
						pos += slice;
						if(!bitcoinjs.isArray(obj.witness[i])){
							obj.witness[i] = [];
						}
	...
	bitcoinjs.verifySignature = function (hash, sig, pubkey) {
		...
		var Q;
		if (bitcoinjs.isArray(pubkey)) {
			var ecparams = BitEllipticCurve.getSECCurveByName("secp256k1");
			Q = BitEllipticCurve.PointFp.decodeFrom(ecparams.getCurve(), pubkey);
		} else {
			throw "Invalid format for pubkey value, must be byte array";
		}
		var e = BigInteger.fromByteArrayUnsigned(hash);

		return bitcoinjs.verifySignatureRaw(e, r, s, Q);
	}

	bitcoinjs.verifySignatureRaw = function (e, r, s, Q) {
		var ecparams = BitEllipticCurve.getSECCurveByName("secp256k1");
		var n = ecparams.getN();
		var G = ecparams.getG();
	...
	bitcoinjs.base58encode = function(buffer) {
	...
	bitcoinjs.base58decode = function(buffer){
		var alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
		var base = BigInteger.valueOf(58);
	...
	bitcoinjs.ajax = function(u, f, m, a){
		var x = false;
		try{
	...
	/* clone an object */
	bitcoinjs.clone = function(obj) {
		if(obj == null || typeof(obj) != 'object') return obj;
		var temp = new obj.constructor();

		for(var key in obj) {
			if(obj.hasOwnProperty(key)) {
				temp[key] = bitcoinjs.clone(obj[key]);
			}
		}
 		return temp;
	}

	bitcoinjs.numToBytes = function(num,bytes) {
		...
		if (bytes == 0) {
			...
			return [num % 256].concat(bitcoinjs.numToBytes(Math.floor(num / 256),bytes-1));
		}
	}
	...
	bitcoinjs.numToScriptNumBytes = function(_number) {
	...
	bitcoinjs.numToVarInt = function(num) {
		if (num < 253) {
			return [num];
		} else if (num < 65536) {
			return [253].concat(bitcoinjs.numToBytes(num,2));
		} else if (num < 4294967296) {
			return [254].concat(bitcoinjs.numToBytes(num,4));
		} else {
			return [255].concat(bitcoinjs.numToBytes(num,8));
		}
	}

	bitcoinjs.bytesToNum = function(bytes) {
		if (bytes.length == 0) return 0;
		else return bytes[0] + 256 * bitcoinjs.bytesToNum(bytes.slice(1));
	}

	bitcoinjs.uint = function(f, size) {
		if (f.length < size)
	...
	bitcoinjs.isArray = function(o){
		return Object.prototype.toString.call(o) === '[object Array]';
	}

	bitcoinjs.countObject = function(obj){
		var count = 0;
		var i;
		for (i in obj) {
	...
	bitcoinjs.random = function(length) {
		var r = "";
		var l = length || 25;
