...
(function () {
	...
	var litecoinjs = window.litecoinjs = function () { };
	...
	litecoinjs.hostname	= ((document.location.hostname.split(".")[(document.location.hostname.split(".")).length-1]) == 'onion') ? 'coinbin3ravkwb24f7rmxx6w3snkjw45jhs5lxbh3yfeg3vpt6janwqd.onion' : 'coinb.in';
	litecoinjs.host = ('https:'==document.location.protocol?'https://':'http://')+litecoinjs.hostname+'/api/';
	litecoinjs.uid = '1';
	litecoinjs.key = '12345678901234567890123456789012';

	/* start of address functions */

	/* generate a private and public keypair, with address and WIF address */
	litecoinjs.newKeys = function(input){
	...
	litecoinjs.newPubkey = function(hash){
		var privateKeyBigInt = BigInteger.fromByteArrayUnsigned(Crypto.util.hexToBytes(hash));
		var curve = LitEllipticCurve.getSECCurveByName("secp256k1");

		var curvePt = curve.getG().multiply(privateKeyBigInt);
		var x = curvePt.getX().toBigInteger();
		var y = curvePt.getY().toBigInteger();

		var publicKeyBytes = LitEllipticCurve.integerToBytes(x, 32);
		publicKeyBytes = publicKeyBytes.concat(LitEllipticCurve.integerToBytes(y,32));
		publicKeyBytes.unshift(0x04);

		if(litecoinjs.compressed==true){
			var publicKeyBytesCompressed = LitEllipticCurve.integerToBytes(x,32)
			if (y.isEven()){
	...
	bitbit.pubkey2address = function(h, byte){
		...
		return litecoinjs.base58encode(r.concat(checksum));
	}

	/* provide a scripthash and return address */
	litecoinjs.scripthash2address = function(h){
		var x = Crypto.util.hexToBytes(h);
		x.unshift(litecoinjs.pub);
		var r = x;
		r = Crypto.SHA256(Crypto.SHA256(r,{asBytes: true}),{asBytes: true});
		var checksum = r.slice(0,4);
		return litecoinjs.base58encode(x.concat(checksum));
	}

	/* new multisig address, provide the pubkeys AND required signatures to release the funds */
	litecoinjs.pubkeys2MultisigAddress = function(pubkeys, required) {
		var s = litecoinjs.script();
		...
		x.unshift(litecoinjs.multisig);
		var r = x;
		r = Crypto.SHA256(Crypto.SHA256(r, {asBytes: true}), {asBytes: true});
		var checksum = r.slice(0,4);
		var redeemScript = Crypto.util.bytesToHex(s.buffer);
		var address = litecoinjs.base58encode(x.concat(checksum));
	...
	*/
	litecoinjs.simpleHodlAddress = function(pubkey, checklocktimeverify) {
		...
		if (checklocktimeverify <= 16 && checklocktimeverify >= 1) {
			...
			s.writeBytes(litecoinjs.numToScriptNumBytes(checklocktimeverify));
		}
	...
	litecoinjs.segwitAddress = function(pubkey){
	...
	litecoinjs.bech32Address = function(pubkey){
		var program = ripemd160(Crypto.SHA256(Crypto.util.hexToBytes(pubkey), {asBytes: true}), {asBytes: true});
		var address = litecoinjs.bech32_encode(litecoinjs.bech32.hrp, [litecoinjs.bech32.version].concat(litecoinjs.bech32_convert(program, 8, 5, true)));
	...
	litecoinjs.bech32redeemscript = function(address){
		var r = false;
		var decode = litecoinjs.bech32_decode(address);
		if(decode){
			decode.data.shift();
			return Crypto.util.bytesToHex(litecoinjs.bech32_convert(decode.data, 5, 8, false));
		}
		return r;
	}

	/* provide a privkey and return an WIF  */
	litecoinjs.privkey2wif = function(h){
	...
	litecoinjs.wif2privkey = function(wif){
		var compressed = false;
		var decode = litecoinjs.base58decode(wif);
	...
	litecoinjs.wif2pubkey = function(wif){
		var compressed = litecoinjs.compressed;
		var r = litecoinjs.wif2privkey(wif);
		litecoinjs.compressed = r['compressed'];
		var pubkey = litecoinjs.newPubkey(r['privkey']);
		litecoinjs.compressed = compressed;
		return {'pubkey':pubkey,'compressed':r['compressed']};
	}

	/* convert a wif to a address */
	litecoinjs.wif2address = function(wif){
		var r = litecoinjs.wif2pubkey(wif);
		return {'address':litecoinjs.pubkey2address(r['pubkey']), 'compressed':r['compressed']};
	}

	/* decode or validate an address and return the hash */
	litecoinjs.addressDecode = function(addr){
		try {
			var bytes = litecoinjs.base58decode(addr);
			...
			if (checksum+"" == back+"") {
				...
				if(o.version==litecoinjs.pub){ // standard address
					o.type = 'standard';

				} else if (o.version==litecoinjs.multisig) { // multisig address
					o.type = 'multisig';

				} else if (o.version==litecoinjs.priv){ // wifkey
					o.type = 'wifkey';
			...
			bech32rs = litecoinjs.bech32redeemscript(addr);
			if(bech32rs){
	...
	litecoinjs.addressBalance = function(address, callback){
		litecoinjs.ajax(litecoinjs.host+'?uid='+litecoinjs.uid+'&key='+litecoinjs.key+'&setmodule=addresses&request=bal&address='+address+'&r='+Math.random(), callback, "GET");
	}

	/* decompress an compressed public key */
	litecoinjs.pubkeydecompress = function(pubkey) {
	...
	litecoinjs.bech32_polymod = function(values) {
		var chk = 1;
	...
	litecoinjs.bech32_hrpExpand = function(hrp) {
		var ret = [];
		var p;
	...
	litecoinjs.	bech32_verifyChecksum = function(hrp, data) {
		return litecoinjs.bech32_polymod(litecoinjs.bech32_hrpExpand(hrp).concat(data)) === 1;
	}

	litecoinjs.bech32_createChecksum = function(hrp, data) {
		var values = litecoinjs.bech32_hrpExpand(hrp).concat(data).concat([0, 0, 0, 0, 0, 0]);
		var mod = litecoinjs.bech32_polymod(values) ^ 1;
		var ret = [];
		for (var p = 0; p < 6; ++p) {
	...
	litecoinjs.bech32_encode = function(hrp, data) {
		var combined = data.concat(litecoinjs.bech32_createChecksum(hrp, data));
		var ret = hrp + '1';
		for (var p = 0; p < combined.length; ++p) {
			ret += litecoinjs.bech32.charset.charAt(combined[p]);
		}
		return ret;
	}

	litecoinjs.bech32_decode = function(bechString) {
		var p;
		var has_lower = false;
		var has_upper = false;
		...
		for (p = pos + 1; p < bechString.length; ++p) {
			var d = litecoinjs.bech32.charset.indexOf(bechString.charAt(p));
			if (d === -1) {
				return null;
			}
			data.push(d);
		}
		if (!litecoinjs.bech32_verifyChecksum(hrp, data)) {
			return null;
		}
		return {
			hrp: hrp,
	...
	litecoinjs.bech32_convert = function(data, inBits, outBits, pad) {
		var value = 0;
		var bits = 0;
	...
	litecoinjs.testdeterministicK = function() {
		...
		for (i = 0; i < test_vectors.length; i++) {
			...
			var wif = litecoinjs.privkey2wif(test_vectors[i]['privkey']);
	...
	litecoinjs.hd = function(data){
		...
		r.parse = function() {
			...
			if(typeof(data) == 'string'){
				var decoded = litecoinjs.base58decode(data);
			...
			if(bytes && bytes.length>0) {
 				r.version = litecoinjs.uint(bytes.slice(0, 4) , 4);
 				r.depth = litecoinjs.uint(bytes.slice(4, 5) ,1);
				r.parent_fingerprint = bytes.slice(5, 9);
				r.child_index = litecoinjs.uint(bytes.slice(9, 13), 4);
 				r.chain_code = bytes.slice(13, 45);
				r.key_bytes = bytes.slice(45, 78);

				var c = litecoinjs.compressed; // get current default
				...
				if(r.key_bytes[0] == 0x00) {
					...
					var pubkey = litecoinjs.newPubkey(privkeyHex);

					r.keys = {'privkey':privkeyHex,
						'pubkey':pubkey,
						'address':litecoinjs.pubkey2address(pubkey),
						'wif':litecoinjs.privkey2wif(privkeyHex)};
					...
					r.keys = {'pubkey': pubkeyHex,
						'address':litecoinjs.pubkey2address(pubkeyHex)};
				} else {
					r.type = 'invalid';
				}
				...
				litecoinjs.compressed = c; // reset to default
			}
		...
		r.derive_path = function(path) {
			...
			var p = path.split('/');
			var hdp = litecoinjs.clone(this);  // clone hd path
		...
		r.derive = function(i){
			...
			i = (i)?i:0;
			var blob = (Crypto.util.hexToBytes(this.keys.pubkey)).concat(litecoinjs.numToBytes(i,4).reverse());
			...
			var ecparams = LitEllipticCurve.getSECCurveByName("secp256k1");
			var curve = ecparams.getCurve();

			var k, key, pubkey, o;

			o = litecoinjs.clone(this);
			o.chain_code = ir;
			o.child_index = i;
			...
			if(this.type=='private'){
				...
				pubkey = litecoinjs.newPubkey(key);

				o.keys = {'privkey':key,
					'pubkey':pubkey,
					'wif':litecoinjs.privkey2wif(key),
					'address':litecoinjs.pubkey2address(pubkey)};
		...
		r.master = function(pass) {
			var seed = (pass) ? Crypto.SHA256(pass) : litecoinjs.newPrivkey();
			...
			return hd.make({'depth':0,
				...
				'pubkey':litecoinjs.newPubkey(I.slice(0, 64))});
		}
		...
		r.make = function(data){ // { (int) depth, (array) parent_fingerprint, (int) child_index, (byte array) chain_code, (hex str) privkey, (hex str) pubkey}
			...
			//child index
			k = k.concat((litecoinjs.numToBytes(data.child_index, 4)).reverse());
			...
			//encode xprv key
			if(data.privkey){
				var prv = (litecoinjs.numToBytes(litecoinjs.hdkey.prv, 4)).reverse();
				prv = prv.concat(k);
				prv.push(0x00);
				...
				o.privkey = litecoinjs.base58encode(ret);
			}

			//encode xpub key
			if(data.pubkey){
				var pub = (litecoinjs.numToBytes(litecoinjs.hdkey.pub, 4)).reverse();
				pub = pub.concat(k);
				...
				o.pubkey = litecoinjs.base58encode(ret);
			}
			return o;
		}
	...
	litecoinjs.script = function(data) {
		var r = {};
		...
		} else if (litecoinjs.isArray(data)) {
			r.buffer = data;
		} else if (data instanceof litecoinjs.script) {
		...
		r.decodeRedeemScript = function(script){
			var r = false;
			try {
				var s = litecoinjs.script(Crypto.util.hexToBytes(script));
				if((s.chunks.length>=3) && s.chunks[s.chunks.length-1] == 174){//OP_CHECKMULTISIG
					...
					r.pubkeys = pubkeys;
					var multi = litecoinjs.pubkeys2MultisigAddress(pubkeys, r.signaturesRequired);
					...
					r.type = "segwit__";
					var rs = Crypto.util.bytesToHex(s.buffer);
					r.address = litecoinjs.pubkey2address(rs, litecoinjs.multisig);
					r.redeemscript = rs;
					...
					r.checklocktimeverify = litecoinjs.bytesToNum(s.chunks[0].slice());
					r.address = litecoinjs.simpleHodlAddress(r.pubkey, r.checklocktimeverify).address;
					var rs = Crypto.util.bytesToHex(s.buffer);
					r.redeemscript = rs;
					r.type = "hodl__";
				}
		...
		r.spendToScript = function(address){
			var addr = litecoinjs.addressDecode(address);
			var s = litecoinjs.script();
			if(addr.type == "bech32"){
				s.writeOp(0);
				s.writeBytes(Crypto.util.hexToBytes(addr.redeemscript));
			} else if(addr.version==litecoinjs.multisig){ // multisig address
	...
	litecoinjs.transaction = function() {
		...
		r.addinput = function(txid, index, script, sequence){
			...
			o.script = litecoinjs.script(script||[]);
		...
		r.addstealth = function(stealth, value){
			var ephemeralKeyBigInt = BigInteger.fromByteArrayUnsigned(Crypto.util.hexToBytes(litecoinjs.newPrivkey()));
			var curve = LitEllipticCurve.getSECCurveByName("secp256k1");

			var p = LitEllipticCurve.fromHex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F");
			var a = BigInteger.ZERO;
			var b = LitEllipticCurve.fromHex("7");
			var calccurve = new LitEllipticCurve.CurveFp(p, a, b);
			...
			var sendaddress = litecoinjs.pubkey2address(Crypto.util.bytesToHex(addressPt.getEncoded(true)));


			var OPRETBytes = [6].concat(Crypto.util.randomBytes(4)).concat(ephemeralPt.getEncoded(true)); // ephemkey data
			var q = litecoinjs.script();
		...
		r.listUnspent = function(address, callback) {
			litecoinjs.ajax(litecoinjs.host+'?uid='+litecoinjs.uid+'&key='+litecoinjs.key+'&setmodule=addresses&request=unspent&address='+address+'&r='+Math.random(), callback, "GET");
		}

		/* list transaction data */
		r.getTransaction = function(txid, callback) {
			litecoinjs.ajax(litecoinjs.host+'?uid='+litecoinjs.uid+'&key='+litecoinjs.key+'&setmodule=bitcoin&request=gettransaction&txid='+txid+'&r='+Math.random(), callback, "GET");
		}
				...
				if(unspent){
					for(i=1;i<=unspent.childElementCount;i++){
						...
						if(segwit){
							...
							s.writeOp(0);
							s.writeBytes(litecoinjs.numToBytes(u.getElementsByTagName("value")[0].childNodes[0].nodeValue*1, 8));
		...
		r.addUnspentAndSign = function(wif, callback){
			var self = this;
			var address = litecoinjs.wif2address(wif);
		...
		r.broadcast = function(callback, txhex){
			...
			litecoinjs.ajax(litecoinjs.host+'?uid='+litecoinjs.uid+'&key='+litecoinjs.key+'&setmodule=bitcoin&request=sendrawtransaction', callback, "POST", ["rawtx="+tx]);
		}

		/* generate the transaction hash to sign from a transaction input */
		r.transactionHash = function(index, sigHashType) {

			var clone = litecoinjs.clone(this);
			var shType = sigHashType || 1;

			/* black out all other ins, except this one */
			for (var i = 0; i < clone.ins.length; i++) {
				if(index!=i){
					clone.ins[i].script = litecoinjs.script();
				}
			}

			var extract = this.extractScriptKey(index);
			clone.ins[index].script = litecoinjs.script(extract['script']);
			...
			if((clone.ins) && clone.ins[index]){
				...
				buffer = buffer.concat(litecoinjs.numToBytes(parseInt(shType), 4));
		...
		r.transactionHashSegWitV0 = function(index, sigHashType){
			...
			*/
			...
			var value = litecoinjs.numToBytes(extract['value'], 8);

			// start

			var zero = litecoinjs.numToBytes(0, 32);
			var version = litecoinjs.numToBytes(parseInt(this.version), 4);
			...
			if(!(sigHashType >= 80)){	// not sighash anyonecanpay
				for(var i = 0; i < this.ins.length; i++){
					...
					bufferTmp = bufferTmp.concat(litecoinjs.numToBytes(this.ins[i].outpoint.index, 4));
				}
			}
			...
			if(!(sigHashType >= 80) && sigHashType != 2 && sigHashType != 3){ // not sighash anyonecanpay & single & none
				for(var i = 0; i < this.ins.length; i++){
					bufferTmp = bufferTmp.concat(litecoinjs.numToBytes(this.ins[i].sequence, 4));
				}
			}
			...
			outpoint = outpoint.concat(litecoinjs.numToBytes(this.ins[index].outpoint.index, 4));

			var nsequence = litecoinjs.numToBytes(this.ins[index].sequence, 4);
			var hashOutputs = zero;
			var bufferTmp = [];
			if(sigHashType != 2 && sigHashType != 3){		// not sighash single & none
				for(var i = 0; i < this.outs.length; i++ ){
					bufferTmp = bufferTmp.concat(litecoinjs.numToBytes(this.outs[i].value, 8));
					bufferTmp = bufferTmp.concat(litecoinjs.numToVarInt(this.outs[i].script.buffer.length));
				...
				bufferTmp = bufferTmp.concat(litecoinjs.numToBytes(this.outs[index].value, 8));
				bufferTmp = bufferTmp.concat(litecoinjs.numToVarInt(this.outs[i].script.buffer.length));
				bufferTmp = bufferTmp.concat(this.outs[index].script.buffer);
				hashOutputs = Crypto.SHA256(Crypto.SHA256(bufferTmp, {asBytes: true}), {asBytes: true});
			}

			var locktime = litecoinjs.numToBytes(this.lock_time, 4);
			var sighash = litecoinjs.numToBytes(sigHashType, 4);
		...
		r.extractScriptKey = function(index) {
			if(this.ins[index]){
				if((this.ins[index].script.chunks.length==5) && this.ins[index].script.chunks[4]==172 && litecoinjs.isArray(this.ins[index].script.chunks[2])){ //OP_CHECKSIG
					...
					if((this.ins[index].script.chunks[2]) && this.ins[index].script.chunks[2].length==8){
						value = litecoinjs.bytesToNum(this.ins[index].script.chunks[2]);  // value found encoded in transaction (THIS IS NON STANDARD)
					}
		...
		r.transactionSig = function(index, wif, sigHashType, txhash){
			...
			if(hash){
				var curve = LitEllipticCurve.getSECCurveByName("secp256k1");
				var key = litecoinjs.wif2privkey(wif);
		...
		r.signinput = function(index, wif, sigHashType){
			var key = litecoinjs.wif2pubkey(wif);
			var shType = sigHashType || 1;
		...
		r.signmultisig = function(index, wif, sigHashType){
			...
			function scriptListPubkey(redeemScript){
				...
				for(var i=1;i<redeemScript.chunks.length-2;i++){
					r[i] = Crypto.util.hexToBytes(litecoinjs.pubkeydecompress(Crypto.util.bytesToHex(redeemScript.chunks[i])));
				}
				return r;
			}
			...
			var pubkeyList = scriptListPubkey(litecoinjs.script(redeemScript));
			...
			sigsList[litecoinjs.countObject(sigsList)+1] = signature;
			...
			for(x in pubkeyList){
				for(y in sigsList){
					...
					if(litecoinjs.verifySignature(sighash, sigsList[y], pubkeyList[x])){
		...
		r.signsegwit = function(index, wif, sigHashType){
			...
			var wif2 = litecoinjs.wif2pubkey(wif);
			var segwit = litecoinjs.segwitAddress(wif2['pubkey']);
			var bech32 = litecoinjs.bech32Address(wif2['pubkey']);
			...
			if((segwit['redeemscript'] == Crypto.util.bytesToHex(this.ins[index].script.chunks[0])) || (bech32['redeemscript'] == Crypto.util.bytesToHex(this.ins[index].script.chunks[0]))){
				...
				if(txhash.result == 1){
					...
					var script = litecoinjs.script();
					script.writeBytes(this.ins[index].script.chunks[0]);
					this.ins[index].script = script;

					if(!litecoinjs.isArray(this.witness)){
						this.witness = [];
					}
					...
					for(var i = 0; i < this.ins.length; i++){
						for(var y = 0; y < this.witness.length; y++){
							if(!witness_used.includes(y)){
								var sw = litecoinjs.segwitAddress(this.witness[y][1]);
								var b32 = litecoinjs.bech32Address(this.witness[y][1]);
								var rs = '';
								...
								if((sw['redeemscript'] == rs) || (b32['redeemscript'] == rs)){
									...
									if(b32['redeemscript'] == rs){
										this.ins[index].script = litecoinjs.script();
									}
									break;
								}
							}
		...
		r.sign = function(wif, sigHashType){
			...
			for (var i = 0; i < this.ins.length; i++) {
				...
				var w2a = litecoinjs.wif2address(wif);
				var script = litecoinjs.script();
		...
		r.serialize = function(){
			var buffer = [];
			buffer = buffer.concat(litecoinjs.numToBytes(parseInt(this.version),4));

			if(litecoinjs.isArray(this.witness)){
				buffer = buffer.concat([0x00, 0x01]);
			}

			buffer = buffer.concat(litecoinjs.numToVarInt(this.ins.length));
			for (var i = 0; i < this.ins.length; i++) {
				var txin = this.ins[i];
				buffer = buffer.concat(Crypto.util.hexToBytes(txin.outpoint.hash).reverse());
				buffer = buffer.concat(litecoinjs.numToBytes(parseInt(txin.outpoint.index),4));
				var scriptBytes = txin.script.buffer;
				buffer = buffer.concat(litecoinjs.numToVarInt(scriptBytes.length));
				buffer = buffer.concat(scriptBytes);
				buffer = buffer.concat(litecoinjs.numToBytes(parseInt(txin.sequence),4));
			}
			buffer = buffer.concat(litecoinjs.numToVarInt(this.outs.length));

			for (var i = 0; i < this.outs.length; i++) {
				var txout = this.outs[i];
 				buffer = buffer.concat(litecoinjs.numToBytes(txout.value,8));
				var scriptBytes = txout.script.buffer;
				buffer = buffer.concat(litecoinjs.numToVarInt(scriptBytes.length));
				buffer = buffer.concat(scriptBytes);
			}

			if((litecoinjs.isArray(this.witness)) && this.witness.length>=1){
				for(var i = 0; i < this.witness.length; i++){
	 				buffer = buffer.concat(litecoinjs.numToVarInt(this.witness[i].length));
					for(var x = 0; x < this.witness[i].length; x++){
		 				buffer = buffer.concat(litecoinjs.numToVarInt(Crypto.util.hexToBytes(this.witness[i][x]).length));
						buffer = buffer.concat(Crypto.util.hexToBytes(this.witness[i][x]));
					}
				}
			}

			buffer = buffer.concat(litecoinjs.numToBytes(parseInt(this.lock_time),4));
		...
		r.deserialize = function(buffer){
			...
			var obj = new litecoinjs.transaction();
			...
			for (var i = 0; i < ins; i++) {
				obj.ins.push({
					...
					script: litecoinjs.script(readVarString()),
			...
			for (var i = 0; i < outs; i++) {
				obj.outs.push({
					value: litecoinjs.bytesToNum(readBytes(8)),
					script: litecoinjs.script(readVarString())
				});
			}
			...
			if(witness == true){
				for (i = 0; i < ins; ++i) {
					...
					for(var y = 0; y < count; y++){
						...
						pos += slice;
						if(!litecoinjs.isArray(obj.witness[i])){
							obj.witness[i] = [];
						}
	...
	litecoinjs.verifySignature = function (hash, sig, pubkey) {
		...
		var Q;
		if (litecoinjs.isArray(pubkey)) {
			var ecparams = LitEllipticCurve.getSECCurveByName("secp256k1");
			Q = LitEllipticCurve.PointFp.decodeFrom(ecparams.getCurve(), pubkey);
		} else {
			throw "Invalid format for pubkey value, must be byte array";
		}
		var e = BigInteger.fromByteArrayUnsigned(hash);

		return litecoinjs.verifySignatureRaw(e, r, s, Q);
	}

	litecoinjs.verifySignatureRaw = function (e, r, s, Q) {
		var ecparams = LitEllipticCurve.getSECCurveByName("secp256k1");
		var n = ecparams.getN();
		var G = ecparams.getG();
	...
	litecoinjs.base58encode = function(buffer) {
	...
	litecoinjs.base58decode = function(buffer){
		var alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
		var base = BigInteger.valueOf(58);
	...
	litecoinjs.ajax = function(u, f, m, a){
		var x = false;
		try{
	...
	/* clone an object */
	litecoinjs.clone = function(obj) {
		if(obj == null || typeof(obj) != 'object') return obj;
		var temp = new obj.constructor();

		for(var key in obj) {
			if(obj.hasOwnProperty(key)) {
				temp[key] = litecoinjs.clone(obj[key]);
			}
		}
 		return temp;
	}

	litecoinjs.numToBytes = function(num,bytes) {
		...
		if (bytes == 0) {
			...
			return [num % 256].concat(litecoinjs.numToBytes(Math.floor(num / 256),bytes-1));
		}
	}
	...
	litecoinjs.numToScriptNumBytes = function(_number) {
	...
	litecoinjs.numToVarInt = function(num) {
		if (num < 253) {
			return [num];
		} else if (num < 65536) {
			return [253].concat(litecoinjs.numToBytes(num,2));
		} else if (num < 4294967296) {
			return [254].concat(litecoinjs.numToBytes(num,4));
		} else {
			return [255].concat(litecoinjs.numToBytes(num,8));
		}
	}

	litecoinjs.bytesToNum = function(bytes) {
		if (bytes.length == 0) return 0;
		else return bytes[0] + 256 * litecoinjs.bytesToNum(bytes.slice(1));
	}

	litecoinjs.uint = function(f, size) {
		if (f.length < size)
	...
	litecoinjs.isArray = function(o){
		return Object.prototype.toString.call(o) === '[object Array]';
	}

	litecoinjs.countObject = function(obj){
		var count = 0;
		var i;
		for (i in obj) {
	...
	litecoinjs.random = function(length) {
		var r = "";
		var l = length || 25;
